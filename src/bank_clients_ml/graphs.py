...
def agregar_a_pdf(pdf, titulo, df, columnas, img_buffer, x_inicial=10, renglon_inicial=45):
    pdf.add_page()
    ...
    for _, row in df[columnas].iterrows():
        ...
        for col in columnas:
            pdf.cell(30, 6, str(row[col]), border=1, new_x=XPos.RIGHT, new_y=YPos.TOP, align="C")
        pdf.ln()
        renglon += 6
...
def Graficar_Variables(pDataFrame, id, tgt, cant_bines, destino, nombre_pdf):
    ...
    for campo in pDataFrame.columns:
        ...
        try:
            print("\n---------------------", campo, "---------------------")
            df, img_buffer, columnas = Graficar_vs_TGT(pDataFrame, campo, id, tgt, cant_bines)
            
            agregar_a_pdf(
                pdf,
                f"Análisis de la variable: {campo}",
                df,
                columnas,
                img_buffer
            )
...
def graficar_top_20(variables_a_graficar):
    ...
    for bar in bars:
        width = bar.get_width()
        ax.text(width + 0.01 * top_vars.max(),
