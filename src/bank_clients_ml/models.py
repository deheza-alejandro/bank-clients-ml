...
param_test = {
    ...
    'max_depth': np.arange(4, 10, 1), # "arange" genera un array de este tipo [4, 5, 6, 7, 8, 9] de 4 a (10-1) aumentando de a 1
...
def generar_split(
    ...
    train, test = train_test_split(
        ...
        stratify=df[target] # mantiene la proporcion de buenos y malos en ambos sets de train y test
    )
...
def process_model_results(df: pd.DataFrame, probabilities: np.ndarray, bins: list[int] = []) -> pd.DataFrame:
    """
    ...
    if bins:
        ...
        combined_df['decil'] = pd.cut(combined_df['Prob1'], bins=bins, labels=decile_labels)
    else:
